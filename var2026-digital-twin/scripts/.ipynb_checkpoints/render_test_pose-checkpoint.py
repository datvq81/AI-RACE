...
# Cấp phép cho PyTorch 2.6 đọc checkpoint
...
def main(config_path, csv_path, output_dir):
    ...
    with torch.no_grad():
        for i, row in enumerate(rows):
            ...
            # 4. ÁP DỤNG ĐỒNG BỘ TỌA ĐỘ TRAIN/TEST (FIX LỖI TOÁN HỌC CHÍ MẠNG)
            # Phải nhân ma trận Dịch chuyển (Transform) trước!
            c2w = global_transform @ c2w
            # Sau đó mới Thu nhỏ (Scale) tọa độ T (Translation)!
            c2w[:3, 3] *= scale
