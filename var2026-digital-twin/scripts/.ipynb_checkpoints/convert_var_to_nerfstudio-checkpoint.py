...
if __name__ == "__main__":
    # Cấu hình quét toàn bộ các thư mục dữ liệu
    DATA_DIRS = ["data/private_test1", "data/public"]
    
    print("\n" + "="*50)
    print(" 🛠️ BẮT ĐẦU CHẠY CÔNG CỤ ĐỒNG BỘ DỮ LIỆU COLMAP")
    print("="*50)
    ...
    for d in DATA_DIRS:
        if os.path.exists(d):
            for s in os.listdir(d):
                ...
                # Chỉ xử lý các thư mục hợp lệ
                if os.path.isdir(scene_path) and not s.startswith('.'):
                    convert_scene(scene_path)
                    total_scenes += 1
                    
    print(f"\n[+] HOÀN TẤT! Đã đồng bộ dữ liệu cho {total_scenes} scenes.")